...
def solve(start, goal="_123456789ABCDEF", heuristic=dist_heuristic, size = 4):
   ...
   h = heuristic(start)
   end_h = heuristic(goal, start)
   visited = {start:[start]}
   end_visited = {goal: [goal]}
   frontier.push((h, start, [start]))
   end_front.push((end_h, goal, [goal]))
   while not frontier.isEmpty() and not end_front.isEmpty():
      s = frontier.pop()
      if s[1] in end_visited.keys():
         return generate_path(start, goal, visited, end_visited[s[1]], s[2])
      ...
      t = end_front.pop() 
      if t[1] in visited.keys():
         return generate_path(goal, start, end_visited, visited[t[1]], t[2])
      for a in generate_children(t[1]):
         a = ''.join(a)
         g = len(t[2]) +1
         h = heuristic(a, start)
         if not a in end_visited: #or visited[a] > h+g:
            end_visited[a] = t[2]+[a]
