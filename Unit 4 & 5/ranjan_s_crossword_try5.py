...
OPENCHAR ='-'
PROTECTEDCHAR = '~'
#args = '13x13 25 H6x4no#on v5x5rot v0x0stride h0x4Trot H0x9arch V0x12heel'.split()         
...
def check_connectivity(board,x, numberblocks, height, width):
    ...
    count = len([z for z in range(len(temp_board)) if temp_board[z] == '?'])
...
def add_blocked_squares(board, number_of_blocks, height, width):
    ...
    temp_board = ''.join(temp_list)
    temp_board, x = block_helper(temp_board, number_of_blocks, height, width) #this is the issue
    if x >= number_of_blocks: 
        board= combine(board, temp_board)
        return board
    temp_board, x = make_palindrome(temp_board, number_of_blocks, height, width)
    if x >= number_of_blocks: 
        board= combine(board, temp_board)
        return board
    #print('after added words:')
    #display(new_board, height, width)
    pos_list = [z for z in range(len(temp_board)) if temp_board[z] == OPENCHAR and temp_board[len(temp_board)-z-1] == OPENCHAR]
    temp_board, x = add_helper(temp_board, number_of_blocks, height, width, x, pos_list)
    temp_board, x = block_helper(temp_board, number_of_blocks, height, width)
    if x == number_of_blocks:
        board = combine(board, temp_board)
        return board
    while check_connectivity(temp_board, x, number_of_blocks, height, width)== False or x !=number_of_blocks or illeg(board,width) ==True: 
         pos_list = [z for z in range(len(temp_board)) if temp_board[z] == OPENCHAR and temp_board[len(temp_board)-z-1] == OPENCHAR]
         temp_board, x = add_helper(temp_board, number_of_blocks, height, width, x, pos_list)
         temp_board, x = block_helper(temp_board, number_of_blocks, height, width)    
    return board
