...
def clustering(img, pix, all_colors, cb, mc, means, count):
...
def main():
   ...
   all_colors = [] #i need a place to keep the colors values
   pix = img.load()   # pix[0, 0] : (r, g, b) 
   print ('Size:', img.size[0], 'x', img.size[1])
   print ('Pixels:', img.size[0]*img.size[1])
   d_count, m_col, m_count, all_colors = distinct_pix_count(img, pix)
   ...
   while not check_move_count(move_count):
      count += 1
      count_buckets, move_count, means = clustering(img, pix, all_colors, count_buckets, move_count, means, count)
      if count == 2:
...
# ll = tk.Label(window, image = img).pack()
   ...
   img.save('km2/2022sranjan.png', 'PNG')  # change to your own filename
   window.mainloop()
   #img.show()
